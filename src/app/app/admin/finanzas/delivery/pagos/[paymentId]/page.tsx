...
export default async function DeliveryPaymentPage({ params }: { params: Promise<{ paymentId: string }> }) {
  ...
  try {
    ...
    const result = p.result as { movementId: number; currency: string; amount: number; rate: number | null; paymentDate: string; reference: string | null; linkedExisting: boolean; orderTotalUsd?: number; extraTotalUsd?: number; extras?: { id: string; date: string; concept: string; amount: number }[] };
