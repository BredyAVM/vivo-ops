  ...
  return <div className="space-y-5">
    ...
    <section className={adminPanel} aria-labelledby="delivery-orders"><h2 id="delivery-orders" className="text-sm font-semibold">Detalle del período</h2>
      <div className="mt-3 divide-y divide-[#292937]">{data.rows.map(row => <article key={row.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
        ...
        <div className="text-right"><p className="text-sm font-semibold tabular-nums">{row.costUsd === null ? 'Costo pendiente' : usd.format(row.costUsd)}</p><p className="mt-1 text-xs text-[#9B9BA7]">{deliveryCostSourceLabel(row.costSource)}</p>{row.costUsd === null ? <Link href={deliveryOrderHref(row)} prefetch={false} className="inline-flex min-h-11 items-center text-xs text-[#FEEF00] underline" aria-label={`Completar costo de delivery de la orden ${row.orderNumber}`}>Completar costo</Link> : null}</div>
