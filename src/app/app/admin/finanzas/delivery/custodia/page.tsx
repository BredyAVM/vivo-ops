...
export default async function DeliveryCustodyPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="space-y-4"><Link href="/app/admin/finanzas/delivery" className="inline-flex min-h-11 items-center text-sm underline">← Servicios y pagos</Link>
      ...
      <section className={adminPanel}>{data.settlements.map(row => <Link key={row.id} href={`/app/admin/finanzas/delivery/${row.id}`} prefetch={false} className="flex min-h-14 items-center justify-between gap-3 border-b border-[#292937] py-3 text-sm"><span>#{row.orderNumber} · {row.responsible}</span><span>{row.status === 'open' ? 'Abierta' : row.status === 'partial' ? 'Parcial' : 'Con diferencia'} →</span></Link>)}
