...
export async function searchMasterOpsOrdersAction(input: {
  ...
  return searchMasterOrdersAction(input);
}
...
export async function confirmMasterOpsPaymentReportAction(input: MasterOpsPaymentConfirmationInput) {
  await requireMasterOrAdminContext();
  return confirmPaymentReportAction({
    ...input,
    clientId: null,
    overrideOperationDate: true,
...
export async function cancelMasterOpsOrderAction(input: Parameters<typeof cancelOrderAction>[0]) {
  await requireMasterOrAdminContext();
  return cancelOrderAction({ ...input, requireExactRefund: true });
}
