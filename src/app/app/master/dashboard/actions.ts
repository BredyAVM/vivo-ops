...
async function appendOrderEvent(
  ...
  input: {
    orderId: number;
    /** Trusted receipt from a command that already wrote the visible history. */
  ...
  try {
    ...
    const { data: insertedEvent, error: insertEventError } = input.persistedEventId
      ? await supabase.from('order_timeline_events').select('id')
          .eq('id', input.persistedEventId).eq('order_id', input.orderId)
          .eq('event_type', input.eventType).single()
      : await supabase
      .from('order_timeline_events')
      .insert({
...
export async function confirmPaymentReportAction(input: PaymentConfirmationInput) {
  const { supabase, user } = await requirePaymentReportOperator();
  const { data, error } = await supabase.rpc('confirm_payment_report_atomic_v1', {
    p_input: buildPaymentConfirmationCommand(input),
  });
  if (error) throw new Error(error.message);
  const receipt = readPaymentConfirmationReceipt(data);
  // Money and the visible event are committed together. Push delivery is best
  // effort and must not turn a completed payment into a user-facing failure.
  if (!receipt.replayed) {
    try {
      const eventContext = await loadOrderEventContext(supabase, receipt.orderId);
      await appendOrderEvent(supabase, {
        orderId: receipt.orderId, context: eventContext,
        persistedEventId: receipt.eventId,
        eventType: 'payment_confirmed', eventGroup: 'payment',
        title: 'Pago confirmado', message: 'El pago reportado fue confirmado.',
        severity: 'info', actorUserId: user.id, payload: receipt.payload,
        recipients: [{ targetRole: 'master' }, { targetUserId: eventContext?.advisorUserId }],
      });
    } catch (notificationError) {
      console.warn('Payment committed; notification delivery pending', notificationError);
    }
  }
...
export async function deliverClientFundChangeAction(input: {
  ...
  return settleClientFundPayoutAction({
    requestId: input.requestId, expectedDifferenceUsd: input.expectedDifferenceUsd,
    orderId: input.orderId, lines: [input], notes: input.notes,
  });
}
...
export async function settleClientFundPayoutAction(input: {
  ...
  try {
    const { supabase } = await requireMasterOrAdmin();
    const { data, error } = await supabase.rpc('settle_client_fund_payout_v1', {
      p_request_id: input.requestId,
      p_input: {
        orderId: input.orderId, expectedDifferenceUsd: input.expectedDifferenceUsd ?? 0,
        notes: input.notes?.trim() || null,
        lines: input.lines.map(line => ({
          moneyAccountId: line.moneyAccountId, currencyCode: line.currencyCode,
          amount: line.amount,
          exchangeRateVesPerUsd: line.currencyCode === 'VES' ? line.exchangeRateVesPerUsd ?? null : null,
          notes: line.notes?.trim() || null,
        })),
      },
    });
    if (error) throw new Error(error.message);
    if (!data || data.groupId !== input.requestId || Number(data.orderId) !== input.orderId
      || !Array.isArray(data.movementIds) || !data.movementIds.length
      || !data.movementIds.every((id: unknown) => Number.isSafeInteger(Number(id)) && Number(id) > 0)
      || !Number.isFinite(Number(data.differenceUsd))) {
      throw new Error('No se pudo verificar el comprobante de la entrega.');
    }
    revalidateMasterDashboardFinancialReferences();
    revalidatePath('/app/advisor');
    return { ok: true as const, groupId: String(data.groupId), differenceUsd: Number(data.differenceUsd) };
  } catch (error) {
    return { ok: false as const, message: error instanceof Error ? error.message : 'Error entregando el fondo.' };
  }
}
...
export async function correctDeliveredDeliveryAssignmentAction(input: {
  ...
  try {
    ...
    const { data, error: correctionError } = await supabase.rpc('correct_delivered_delivery_v1', {
      p_order_id: input.orderId,
      p_kind: input.assignmentKind,
      p_driver_user_id: input.assignmentKind === 'internal' ? input.driverUserId ?? null : null,
      p_partner_id: input.assignmentKind === 'external' ? input.partnerId ?? null : null,
      p_reference: input.reference ?? null,
      p_distance_km: input.distanceKm ?? null,
      p_cost_usd: parseDeliveryCostInput(input.costUsd),
      p_notes: notes,
    });
    if (correctionError) throw new Error(correctionError.message);
    const receipt = readDeliveryCorrectionReceipt(data);
    // Business change and visible history are already committed. Notifications are best effort.
    try {
      const eventContext = await loadOrderEventContext(supabase, input.orderId);
      await appendOrderEvent(supabase, {
        orderId: input.orderId,
...
export async function createInventoryItemAction(input: {
  ...
  if (!['raw', 'fried', 'prefried', 'sauces', 'beverages', 'packaging', 'other'].includes(input.inventoryGroup)) {
...
export async function closeOrderRoundingBalanceAction(input: {
  ...
  const { data: financialRows, error: financialError } = await supabase.rpc('get_order_financial_state', {
    p_order_id: orderId,
  });
  const financial = Array.isArray(financialRows) ? financialRows[0] : financialRows;
  if (financialError || !financial) throw new Error(financialError?.message || 'No se pudo consultar el saldo de la orden.');
  const confirmedPaidUsd = roundMoney(financial.confirmed_paid_usd);
  const currentTotalUsd = roundMoney(financial.total_usd);
  const currentTotalBs = roundMoney(financial.total_bs);
  const pendingUsd = roundMoney(financial.pending_usd);
...
export async function voidMoneyMovementGroupAction(input: {
  ...
  const result = await voidFinancialMovementAction(input);
...
export async function voidFinancialMovementAction(input: {
  ...
  try {
    const { supabase, roles } = await requireMasterOrAdmin();
    requireAdminRole(roles);
    const { data, error } = await supabase.rpc('void_financial_movement_v1', {
      p_movement_id: input.movementId,
      p_group_id: input.movementGroupId || null,
      p_reason: input.reason,
    });
    if (error) throw new Error(error.message);
    if (!data || !Array.isArray(data.movementIds) || data.movementIds.length === 0
      || !data.movementIds.every((id: unknown) => Number.isSafeInteger(Number(id)) && Number(id) > 0)
      || !Array.isArray(data.paymentReportIds)) {
      throw new Error('No se pudo verificar la anulación. Reintenta sin cambiar los datos.');
    }
    revalidateMasterDashboardFinancialReferences();
    return { ok: true as const, movementIds: data.movementIds as number[], paymentReportIds: data.paymentReportIds as number[] };
  } catch (error) {
    return { ok: false as const, message: error instanceof Error ? error.message : 'No se pudo anular el movimiento.' };
  }
}
...
export async function createMoneyAccountClosureAction(input: {
  ...
  try {
    ...
    if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(requestId || '')) {
      throw new Error('Actualiza la pantalla antes de registrar el cierre.');
    }
    const { data, error } = await supabase.rpc('create_account_closure_v1', {
      p_request_id: requestId, p_input: command,
    });
    if (error) throw new Error(error.message);
    if (!data || !Number.isSafeInteger(Number(data.closureId)) || Number(data.closureId) <= 0) {
      throw new Error('No se pudo verificar el cierre. Reintenta sin cambiar los datos.');
    }
    revalidateMasterDashboardFinancialReferences();
    return { ok: true as const };
  } catch (error) {
    return { ok: false as const, message: error instanceof Error ? error.message : 'No se pudo registrar el cierre.' };
  }
}
...
export async function previewMoneyAccountClosureAction(input: {
  ...
  const { supabase } = await requireMasterOrAdmin();
  return loadAccountClosurePreview(supabase, input);
}
...
export async function rejectMoneyAccountClosureAction(input: {
  ...
  const { supabase, roles } = await requireMasterOrAdmin();
  requireAdminRole(roles);
  const { data, error } = await supabase.rpc('void_account_closure_v1', {
    p_closure_id: input.closureId, p_reason: input.reason,
  });
  if (error) throw new Error(error.message);
  if (!data || Number(data.closureId) !== input.closureId) throw new Error('No se pudo verificar la anulación del cierre.');
  revalidateMasterDashboardFinancialReferences();
}
...
export async function resolveMoneyAccountReconciliationItemAction(input: {
  ...
  const { supabase } = await requireMasterOrAdmin();
  if (input.resolutionMode && input.resolutionMode !== 'note_only') {
    throw new Error('Para registrar o vincular dinero usa Administración → Cuentas → Conciliación → Resolver. Allí se verifica la evidencia antes de guardar.');
  }
  const { data, error } = await supabase.rpc('admin_account_reconciliation_detail_v1', { p_item_id: input.itemId });
  if (error || !data?.item) throw new Error(error?.message || 'Diferencia no encontrada.');
  const { error: resolveError } = await supabase.rpc('resolve_account_reconciliation_v1', {
    p_request_id: crypto.randomUUID(),
    p_input: { itemId: input.itemId, mode: 'note_only', amount: data.item.amount,
      fingerprint: data.fingerprint, note: input.resolutionNotes },
  });
