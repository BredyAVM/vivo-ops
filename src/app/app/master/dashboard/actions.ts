...
export async function voidFinancialMovementAction(input: {
  ...
  try {
    const { user, roles, supabase: sessionClient } = await requireMasterOrAdmin();
    if (!getMasterDashboardPermissions(roles).isAdmin) {
    ...
    const { data: transferReceipt, error: transferVoidError } = await sessionClient.rpc('void_money_transfer_v1', {
      p_movement_id: movementId,
      p_group_id: movementGroupId,
      p_reason: reason,
    });
    if (transferVoidError) throw new Error(transferVoidError.message);
    if (transferReceipt) {
      revalidateMasterDashboardFinancialReferences();
      return { ok: true as const, movementIds: transferReceipt.movementIds as number[], paymentReportIds: [] as number[] };
    }
