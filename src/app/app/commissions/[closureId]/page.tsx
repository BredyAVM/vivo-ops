            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                {snapshot.gifts.length === 0 ? <EmptyDetail>No hubo obsequios en este cierre.</EmptyDetail> : (
                  <TableFrame><table className="min-w-[920px] w-full text-left text-xs"><thead className="bg-[#0F0F13] text-[10px] uppercase tracking-[0.12em] text-[#858591]"><tr><th className="px-4 py-3">Pedido</th><th className="px-4 py-3">Cliente</th><th className="px-4 py-3">Obsequio</th><th className="px-4 py-3">Jugada</th><th className="px-4 py-3 text-right">Cantidad</th><th className="px-4 py-3 text-right">Aporte empresa</th><th className="px-4 py-3 text-right">Cargo asesor</th></tr></thead><tbody className="divide-y divide-[#292933]">{snapshot.gifts.map((gift: CommissionAuditGift, index) => <tr key={`${gift.orderId || gift.orderNumber || 'gift'}-${index}`}><td className="px-4 py-3 font-semibold">{orderLabel(gift)}</td><td className="px-4 py-3">{gift.clientName || 'Cliente'}</td><td className="px-4 py-3">{gift.productName || 'Obsequio'}</td><td className="px-4 py-3">{gift.crmPlayName || 'Catálogo general'}</td><td className="px-4 py-3 text-right">{commissionAuditNumber(gift.qty)}</td><td className="px-4 py-3 text-right">{gift.companyCostUsd == null ? '—' : money(gift.companyCostUsd)}</td><td className="px-4 py-3 text-right font-semibold text-amber-200">{money(gift.deductionUsd)}</td></tr>)}</tbody></table></TableFrame>
                )}
              </div>
              <div>
