-- Inventory availability is advisory for order operations. A physical shortage
-- must remain visible in the canonical ledger, but it must not roll back an
-- order that Master has actually delivered.
...
begin
  ...
  -- Deliberately do not reject insufficient stock here. The sale is a physical
  -- fact and must be recorded. A negative balance exposes the discrepancy to
  -- the existing procurement/availability alerts instead of blocking delivery.
  for v_line in
    select
...
comment on function public.inventory_commit_order_sale_v1(uuid, bigint, text) is
  'Atomic non-blocking sale command. Records delivered-order consumption even when the resulting physical balance is negative.';
