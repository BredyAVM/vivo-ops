-- Counter client search: expose the existing primary advisor when present,
-- otherwise the advisor from the client's most recent attributed order.
-- The lookup runs only for the small, paginated search result set.
...
begin
  ...
  page_with_advisor as materialized (
    select
      page.*,
      coalesce(page.primary_advisor_id, recent_order.attributed_advisor_id) as advisor_user_id,
      coalesce(primary_profile.full_name, recent_profile.full_name) as advisor_name,
      case
        when page.primary_advisor_id is not null then 'primary'
    ...
    left join lateral (
      ...
      where order_row.client_id = page.id
        and order_row.attributed_advisor_id is not null
...
is 'Busqueda paginada Counter con asesor primario existente o, como contexto, el asesor de la ultima orden atribuida.';
