...
begin
  ...
  select round(coalesce(sum(public.counter_amount_usd(
    ...
    case when upper(btrim(line ->> 'currency_code')) = 'VES'
      then round((line ->> 'exchange_rate_ves_per_usd')::numeric, 6)
      else null
    end
  )), 0), 2)
  into v_expected_usd
  from jsonb_array_elements(coalesce(p_expected_collection_lines, '[]'::jsonb)) line;
  ...
  from jsonb_array_elements(coalesce(p_cash_change_lines, '[]'::jsonb)) line;
  ...
  from jsonb_array_elements(coalesce(p_digital_change_lines, '[]'::jsonb)) line;
