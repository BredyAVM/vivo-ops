...
is 'Autorizacion operacional para modificar un pickup que Counter ya recibio listo o empacado.';
    ...
    select
      ...
      case
        when priced.source_currency = 'VES'
          then round(priced.source_amount / v_rate, 2)
        else round(priced.source_amount, 2)
      end as unit_usd,
      case
        when priced.source_currency = 'VES'
  ...
  loop
    if (v_item ->> 'qty')::numeric = 0 then
      ...
      set
        ...
        line_total_usd = (v_item ->> 'lineUsd')::numeric,
...
begin
  ...
  select v_event_id, 'kitchen', null, false
  union all
...
begin
  ...
  if v_order.status = 'ready' and char_length(v_reason) < 4 then
    raise exception 'A clear reason is required to request a change to a ready pickup';
  end if;
  ...
  if v_order.status = 'ready' then
    ...
    insert into public.order_timeline_events (
      ...
      'approval',
      'Cambio de pickup listo requiere autorizacion',
      v_reason,
      'warning',
      v_uid,
      jsonb_build_object(
    ...
    select v_event_id, 'master', null, true
    union all
    select v_event_id, 'advisor', v_order.attributed_advisor_id, false
    where v_order.attributed_advisor_id is not null;
...
begin
  ...
  if p_decision = 'reject' then
    ...
    select v_event_id, 'counter', null, false
    union all
    select v_event_id, 'advisor', v_order.attributed_advisor_id, false
    where v_order.attributed_advisor_id is not null;
  ...
  if v_order.status <> 'ready'
     or v_current_signature is distinct from v_request.base_signature then
    v_result := jsonb_build_object(
      'status', 'stale',
      'requestId', p_request_id,
      'orderId', v_request.order_id,
...
begin
  ...
  select v_event_id, 'master', null, false
  union all
  select
    v_event_id,
    'advisor',
